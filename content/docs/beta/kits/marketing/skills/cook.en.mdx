...
| `--advice` | Adds `kongming` advisory checkpoints with host-specific routing: Fable 5 for Claude subscriptions, `gpt-5.6-sol` with high effort for Codex, and `claude-fable-5-high` for Cursor. Honor a plan Failure Protocol when present and stop for `kongming` on the first unexpected failed verification before self-reasoning | Advice does not implement, approve, or expand authority |
