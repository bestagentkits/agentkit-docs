...
On POSIX, a wrapper-managed Codex home may link only the final `hooks` leaf to
the resolved user's canonical `~/.codex/hooks`. AgentKit uses that canonical
path for translated commands, writes, rollback, and ownership. Linked parents,
noncanonical targets, and all Windows links or reparse points remain refused.
