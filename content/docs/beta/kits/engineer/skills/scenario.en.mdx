...
Irrelevant dimensions should be listed with both a reason and the assumption
behind the skip, not silently counted as covered. If that assumption could stop
being true during the feature's life, treat its failure as a scenario instead
of skipping the dimension.
...
2. **The Skill filters dimensions.** Relevant dimensions become the exploration
   plan; skipped dimensions remain visible with their assumptions. A breakable
   skip assumption becomes a failure scenario.
...
for [`ak eval workflow`](../../../reference/cli/eval/workflow) with workflow `scenario`. The default
...
permission. Existing caps, your stops, human checkpoints, and approval controls remain authoritative. The generator still creates candidates and checks the complete history; the request carries at most the latest 16 kept situations plus the total history count, and partial history cannot qualify for a changed classification. The novelty and value scores are ordinal, not percentages, and KEEP or DISCARD describes a candidate, not whether the loop continues. Ordinary use of the Skill makes no call.
