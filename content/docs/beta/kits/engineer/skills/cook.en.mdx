...
| `--advice` | Adds advisory checkpoints for phase results, blockers, high-stakes decisions, and the first unexpected failed verification | Advice does not write code, approve gates, or replace the main implementation workflow |
...
When `--advice` is selected, `kongming` uses Fable 5 for Claude subscription
sessions, `gpt-5.6-sol` with `high` effort for Codex, and
`claude-fable-5-high` for Cursor. The Skill also honors a plan Failure Protocol
when present and stops for `kongming` on the first unexpected failed
verification before self-reasoning. When `--yagni` is selected, the Skill
passes the literal flag to downstream Skills and delegated work so the opt-in
survives each handoff.
