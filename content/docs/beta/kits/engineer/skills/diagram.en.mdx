...
Describe topology and semantic roles; the compiler owns layered placement and orthogonal edge routing. Read the role legend to interpret nodes. The default finite entrance completes in under eight seconds; optional trace motion is also finite. Use Replay or `M` to replay, and reduced-motion preferences render the final frame directly. `--step` is an internal CSS custom property, not an `ak` CLI flag.
