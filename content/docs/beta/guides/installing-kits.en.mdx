...
`--target cursor`, `--target grok`, or `--target omp`. Claude Code is the
default when you omit the flag. Released registry packages are resolved for one
runtime at a time, so run a separate install for each runtime you use. Oh My Pi
can also be selected by `ak run --target omp` for Skill dispatch.
...
| Oh My Pi project | Project | Native | Oh My Pi package staging plus project lifecycle metadata |
| Oh My Pi user | Global | Native | Oh My Pi package staging plus user-scope lifecycle metadata |
