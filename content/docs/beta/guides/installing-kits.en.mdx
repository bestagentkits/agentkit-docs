...
does not create installed lifecycle state. The previewed destination is the
authority when an environment overrides a normal runtime root.
...
means deleting an entire `.claude`, `.codex`, or user runtime directory.
