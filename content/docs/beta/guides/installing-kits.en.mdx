...
`--target cursor`, or `--target grok`. Claude Code is the default when you omit
the flag. Released registry packages are resolved for one runtime at a time, so
run a separate install for each runtime you use. Oh My Pi is a harness dispatch
target for `ak run --target omp`; it is not an `ak kit init` install target.
