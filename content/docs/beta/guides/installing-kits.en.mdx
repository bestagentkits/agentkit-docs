...
`--target cursor`, `--target dsh`, `--target grok`, `--target pi`, or `--target omp`. Claude
...
flag on `ak kit init` and `ak kit refresh` uses persisted `kit.channel` when
that key is set; otherwise it resolves `stable`, which can return
`version_not_available` when the selected runtime package exists only on another
channel. `ak kit install` still resolves `stable` when the flag is omitted.
...
Install one Kit into a project-local Pi projection or the active Pi coding-agent
profile:
...
and `extensions/` directly below that profile. A fresh global install refuses an
occupied profile by default. If you have reviewed the listed paths and intend
to overwrite them, re-run with explicit `--force`.
...
For optional web access, code feedback, delegation, and other additions, see
[Recommended Pi extensions](./pi-extensions).
...
Project installs write portable Skills to
`.omp/skills/ak-<slug>/SKILL.md`; each projected file uses top-level
`name: ak-<slug>`. Agents go to `.omp/agents/<name>.md`, and rules go to
`.omp/rules/<name>.md`. Body aliases such as `/ak:cook` and `$ak:cook` remain
unchanged. Kit instructions are merged
...
Trusted project OMP also discovers project `.agents/skills`. For a canonical
collision such as `ak:advise` and `ak-advise`, project `.omp/skills` shadows the
project `.agents/skills` copy after a project OMP install. When a global OMP
install is used from that trusted project, project `.agents/skills` shadows the
OMP profile copy. The install warns and continues without skipping OMP emission,
deleting or rewriting either Skill, changing `.agents` or OMP settings, requiring
`--force`, or returning exit `6`.
...
Kit installation still uses the `stable` registry channel when `--channel` is
omitted. `ak kit init`, `ak kit refresh`, and `ak update` use persisted
`kit.channel` when that key is set. To try a prerelease kit, opt into `beta`
explicitly and keep the CLI on a compatible channel:
...
fail closed or remain preserved. The bounded exception is a Codex `hooks` leaf
that points exactly to the resolved user's canonical `~/.codex/hooks`, whether
through a POSIX symlink or a Windows directory junction; AgentKit uses that
canonical identity for the complete Hook lifecycle. Linked parents and other
targets still fail closed. Ownership evidence qualifies cleanup; it never means deleting
...
A successful Codex install can report both **Hook groups not projected** and **Hook matchers narrowed (some tool matches unsupported on this
...
For global Codex Hooks, a wrapper-managed Codex home may alias only its final
`hooks` leaf to canonical `~/.codex/hooks`, using a POSIX symlink or a Windows
directory junction. Do not generalize this to a linked Codex home or arbitrary
Hook target; those remain refused.
...
DeepSeek Harness is a signed registry target. Use `--target dsh`; project Skills go under `.dsh/skills`, user Skills under `${DSH_HOME:-~/.dsh}/skills`. Install scope, dispatch and live verification are separate capabilities. See the [runtime matrix](../concepts/runtime-adapters#runtime-capability-matrix).
