...
Luồng này sao chép các giá trị được hỗ trợ, giữ giá trị AgentKit hiện có, bỏ qua
...
<Callout type="warn" title="Rollback bắt đầu ngay">
...
Chỉ chạy rollback sau khi kiểm tra bản sao lưu độc lập và sao chép riêng công
việc không liên quan được tạo sau migration:

```bash
ak migrate rollback
```

Với legacy transition, hành vi phụ thuộc phase đã ghi nhận. Trước khi bản cài
...
Rollback chỉ xử lý trạng thái đã ghi nhận. Đây không phải thao tác hoàn tác có
chọn lọc các chỉnh sửa về sau, không dựng lại toàn bộ môi trường trước apply và
không gỡ bản thay thế AgentKit đã cài. Nếu không còn muốn bản thay thế, hãy
preview rồi uninstall bằng đúng target, scope và delivery route.
