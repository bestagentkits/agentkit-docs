...
`--target cursor`, `--target grok` hoặc `--target omp`. Nếu bỏ qua flag này, mặc
định là Claude Code. Package từ registry được resolve cho từng runtime, vì vậy
hãy chạy một lệnh cài riêng cho mỗi runtime bạn sử dụng. Oh My Pi cũng có thể
được chọn bằng `ak run --target omp` cho Skill dispatch.
...
| Oh My Pi trong project | Project | Native | Package staging cho Oh My Pi cùng metadata vòng đời của project |
| Oh My Pi cho người dùng | Global | Native | Package staging cho Oh My Pi cùng metadata vòng đời ở user scope |
...
ak kit init engineer --target grok --global
