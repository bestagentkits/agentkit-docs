...
`--target cursor`, `--target dsh`, `--target grok`, `--target pi` hoặc `--target omp`. Nếu bỏ qua flag này, mặc
...
trên `ak kit init` và `ak kit refresh`, lệnh dùng `kit.channel` đã lưu khi khóa
đó đã được đặt; nếu không thì resolve `stable`. Target chỉ có package ở kênh
khác có thể trả `version_not_available`. `ak kit install` vẫn resolve `stable`
khi bỏ cờ.
...
`rules/` và `extensions/` dưới profile đó. Theo mặc định, bản cài global mới sẽ
từ chối profile đã có nội dung. Nếu bạn đã xem các đường dẫn được liệt kê và
muốn ghi đè, hãy chạy lại với `--force` rõ ràng.
...
Để bổ sung truy cập web, phản hồi mã nguồn, giao việc và các khả năng khác, xem
[Các extension Pi được đề xuất](./pi-extensions).
...
Bản cài project ghi Skill portable vào
`.omp/skills/ak-<slug>/SKILL.md`; mỗi file đã projection dùng `name: ak-<slug>` ở
top-level. Agent được ghi vào `.omp/agents/<name>.md`, còn rule vào
`.omp/rules/<name>.md`. Alias trong body như `/ak:cook` và `$ak:cook` được giữ
nguyên. Instruction của Kit được merge
...
OMP trong project đã trust cũng discover `.agents/skills` của project. Với xung
đột canonical như `ak:advise` và `ak-advise`, `.omp/skills` của project che bản
trong `.agents/skills` sau bản cài OMP project. Khi dùng bản cài OMP global từ
project đã trust đó, `.agents/skills` của project che bản trong profile OMP. Lệnh
cài cảnh báo rồi tiếp tục mà không bỏ qua emission OMP, xóa hoặc viết lại Skill,
thay đổi `.agents` hay setting OMP, yêu cầu `--force` hoặc trả exit `6`.
...
`ak kit install` vẫn dùng kênh registry `stable` khi bỏ `--channel`. `ak kit
init`, `ak kit refresh` và `ak update` dùng `kit.channel` đã lưu khi khóa đó đã
được đặt. Để thử Kit prerelease, hãy chủ động chọn `beta` và giữ CLI trên kênh
tương thích:
...
thường fail closed hoặc được giữ lại. Exception có giới hạn là leaf `hooks` của
Codex trỏ chính xác tới `~/.codex/hooks` canonical của user đã resolve, qua
symlink POSIX hoặc directory junction trên Windows; AgentKit dùng identity
canonical đó cho toàn bộ lifecycle Hook. Parent có link và target khác vẫn fail
closed.
Bằng chứng ownership giới hạn cleanup; nó không bao giờ có nghĩa xóa toàn bộ
`.claude`, `.codex`, `.grok` hoặc thư mục runtime của người dùng.
...
Một lần cài Codex thành công có thể báo cả **Hook groups not projected** và **Hook matchers narrowed (some tool matches unsupported on this
...
Với Hook Codex global, Codex home do wrapper quản lý chỉ được alias leaf `hooks`
cuối cùng tới `~/.codex/hooks` canonical, bằng symlink POSIX hoặc directory
junction trên Windows. Đừng mở rộng exception này sang Codex home có link hoặc
target Hook tùy ý; các trường hợp đó vẫn bị từ chối.
...
DeepSeek Harness là target registry đã ký. Dùng `--target dsh`; Skill project ở `.dsh/skills`, Skill user ở `${DSH_HOME:-~/.dsh}/skills`. Scope cài đặt, dispatch và live verification là các capability riêng. Xem [bảng runtime](../concepts/runtime-adapters#runtime-capability-matrix).
