...
Codex trên POSIX trỏ chính xác tới `~/.codex/hooks` canonical của user đã
resolve; AgentKit dùng identity canonical đó cho toàn bộ lifecycle Hook. Parent
có link, target khác và link hay reparse point trên Windows vẫn fail closed.
...
Với Hook Codex global trên POSIX, Codex home do wrapper quản lý chỉ được alias
leaf `hooks` cuối cùng tới `~/.codex/hooks` canonical. Đừng mở rộng exception
này sang Codex home có link hoặc target Hook tùy ý; các trường hợp đó vẫn bị từ
chối, cũng như mọi link và reparse point trên Windows.
