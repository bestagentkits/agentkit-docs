...
Một lần cài Codex thành công có thể báo
`Hooks dropped (unsupported on this target)`. Trong bản phát hành này, Codex
không biểu diễn được nhóm hook `PostToolUse` của Engineer Kit với matcher trạng
thái task `Agent|Task|TodoWrite|TodoRead`, nên nhóm đó không được phát sang
Codex. Các nhóm hook tương thích vẫn được cài, còn việc làm mới trạng thái task
vẫn chạy ở các ranh giới `Stop` và `SubagentStop` được hỗ trợ.

Trong output JSON, hãy kiểm tra `hooksDropped` và `droppedHookSummaries`. Hook bị
bỏ có thông báo là giới hạn capability của runtime, không phải lỗi cài đặt; cài
lại với `--force` không bổ sung matcher chưa được hỗ trợ.
