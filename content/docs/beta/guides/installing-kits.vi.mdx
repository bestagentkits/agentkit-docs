...
`--target cursor` hoặc `--target grok`. Nếu bỏ qua flag này, mặc định là Claude
Code. Package từ registry được resolve cho từng runtime, vì vậy hãy chạy một
lệnh cài riêng cho mỗi runtime bạn sử dụng. Oh My Pi là harness dispatch target
cho `ak run --target omp`; đây không phải target cài của `ak kit init`.
