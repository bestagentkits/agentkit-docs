...
<Callout type="warning">
  Grok Build là ngoại lệ: `--target grok` native vẫn là `spike` dùng nguồn cục
  bộ, không phải target production hoặc có thể lấy từ remote. Hãy dùng nguồn
  `--local --kits-dir` rõ ràng; registry không có package runtime Grok đã ký.
</Callout>
...
mỗi runtime bạn sử dụng. `--target grok` native là một `spike` dùng nguồn cục bộ
riêng, được mô tả bên dưới.
...
| Grok Build trong project (`spike`) | Project | Native | `<project>/.grok` cùng `<project>/.agentkit/adapters/grok/<kit>` |
| Grok Build cho người dùng (`spike`) | Global | Native | `${GROK_HOME:-~/.grok}` cùng `${AGENTKIT_HOME:-~/.agentkit}/adapters/grok/<kit>` |

Cursor và AGY dùng bề mặt riêng của target, còn `portable` là export và không tạo
trạng thái vòng đời đã cài. Các dòng Grok chỉ áp dụng cho đường dùng nguồn cục bộ
rõ ràng. Đích hiển thị trong preview là nguồn chính xác khi môi trường ghi đè
runtime root thông thường.
...
### Dùng Grok Build spike native

Dùng nguồn Kit cục bộ rõ ràng cho development hoặc CI:
...
Init và refresh Grok từ remote fail closed vì chưa có package runtime Grok đã
ký. Không thay bằng package Claude Code, Codex hoặc Cursor. Bản cài project dùng
`<project>/.grok`; bản cài global dùng `${GROK_HOME:-~/.grok}`. Quyền sở hữu
lifecycle nằm trong tuyến `.agentkit/adapters/grok/<kit>` riêng được nêu ở trên.
...
snapshot trước các thay đổi phá hủy. Grok Build `spike` thay vào đó yêu cầu nguồn
