...
<Callout type="warn" title="ak update has no --no-backup flag">
  The default preview and explicit `--dry-run` do not write files or create a
  snapshot, but they are previews, not an apply-without-backup mode. An
  applied update always takes the pre-update snapshot first; `--force`
  overwrites user-modified files but does not skip that snapshot. `--no-backup`
  exists only on [`ak init`](../reference/cli/init),
  ...
  warning on those pages before using it.
</Callout>
