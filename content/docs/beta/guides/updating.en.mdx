...
<Callout type="warn">
  `ak update` has no `--no-backup` flag. The pre-update snapshot on an applied
  project update (`--yes`) is not optional — `--force` overwrites
  user-modified files but does not skip it. The default preview and explicit
  `--dry-run` write nothing and take no snapshot; they are a preview, not an
  apply-without-backup mode. Commands that do expose `--no-backup` —
  [`ak init`](../reference/cli/init), [`ak kit init`](../reference/cli/kit/init),
  [`ak kit install`](../reference/cli/kit/install), and
  [`ak kit refresh`](../reference/cli/kit/refresh) — carry their own
  data-loss warning.
</Callout>
