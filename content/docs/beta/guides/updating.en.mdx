...
but have not installed globally is reported as skipped rather than created. Use
`--target` or its `--runtime` alias to select installed runtimes such as
`claude-code`, `codex`, `cursor`, `grok`, or `omp`; pass only one of the two flag names.
The same implicit-preview exit `3` and explicit-dry-run exit `0` distinction
applies at global scope.
