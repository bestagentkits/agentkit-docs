...
| `omp` | `AGENTKIT_OMP_BIN` or `omp` on `PATH`; routes through Oh My Pi. | Provider gate is skipped; Oh My Pi owns its configured backend and credentials. |
...
<Callout type="warning">
  ...
  `ak run --target omp` is local harness dispatch through Oh My Pi; it is not a
  Kit install target.
</Callout>
