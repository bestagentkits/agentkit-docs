---
title: ak run
description: Resolve a local Kit skill and stream it through Claude Code, Codex, Cursor, or the Grok Build spike with explicit stdin, timeout, output, and dependency behavior.
---
...
| --- | --- | --- |
| `--target <adapter>` | `claude-code` | Select `claude-code`, `codex`, `cursor`, or `grok`. |
...
Route one invocation through the registered Grok adapter:
...
<Callout type="warning">
  Grok Build remains a local-development `spike`. `ak run --target grok` uses
  the source-registered local adapter, but no authenticated provider success
  canary has shipped. AgentKit does not sign in to Grok or choose its model.
</Callout>
