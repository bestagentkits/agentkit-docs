...
Global output also includes `locations`, `kits`, and `skipped_targets`. Each Kit result can report `kit`, `runtime`, `status`, `channel`, `reason`, `error`, `why`, `next`, snapshot fields, `locations`, and `warnings`. A selected runtime that is not installed globally is reported as skipped rather than created.
...
On Windows, `ak update` applies the signed CLI replacement in place under an exclusive update lock, validates the running binary preimage, then re-executes the original command inside a Windows Job Object so Kit refresh continues on the first invocation. Standalone [`ak self-update`](./self-update) still uses the staged detached helper handoff. Windows paired Desktop apply remains rejected; download and verify the Desktop zip separately.
