...
- With `--include-desktop --yes` on a supported platform, matching CLI and GUI
  artifacts are staged as one pending transaction. On macOS and Linux, restart
  the AgentKit Desktop app to reverify and complete it. On Windows x64, a
  trusted handoff helper commits the verified swap after the parent process
  releases the executables.
