...
Route một invocation qua adapter Grok đã đăng ký, hoặc qua harness dispatch target Oh My Pi. Projection Kit Pi không phải adapter `ak run` trong bản phát hành này; dùng `ak kit init ... --target pi` cho Skill cài vào Pi và dùng discovery riêng của Pi để gọi chúng.
