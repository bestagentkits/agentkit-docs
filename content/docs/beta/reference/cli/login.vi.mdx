...
<Callout type="info" title="API key và license key khác nhau">
  `--api-key` dành cho chủ sở hữu Kit: xác thực CLI bằng user API key gắn với
  một Kit đã mua trực tiếp, seat trong team hoặc suất waitlist trả phí.
  `--license-key` dành cho AgentKit App: kích hoạt device session từ App
  license và bản thân nó không cấp quyền CLI truy cập Kit. Một tài khoản có
  thể có một trong hai, cả hai, hoặc không có cái nào; hãy xác minh bằng
  `ak whoami --json` và `ak licenses --json`, xem thêm
  [Xử lý sự cố xác thực](../../troubleshooting/authentication) để biết các lỗi
  từ chối cụ thể.
</Callout>
