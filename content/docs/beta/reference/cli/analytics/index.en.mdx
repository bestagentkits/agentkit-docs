...
SQLite analytics index. The index is a derived cache; local session and
activity files remain authoritative. The CLI never turns it on by itself, but
the desktop and dashboard hosts do unless you have disabled or deleted it.
...
The desktop app and the dashboard started by `ak config start` run a host
maintenance pass. It enables and builds an index that was never set up, and it
deletes and rebuilds an index AgentKit wrote that still reports `corruption` or
`migration-error` after the ordinary migration pass, at most once an hour. It
never overrides an explicit `disable` or `delete`. No CLI command does this, so
a scripted `ak` run never creates analytics state it was not asked for.
