...
```bash
ak setup --adapter claude-code,codex,cursor,grok,omp --no-interactive
...
Chọn `grok` sẽ ghi tuỳ chọn enabled-adapter và kiểm tra binary cục bộ có resolve
được hay không. Setup không cài package Kit, probe hoặc ghi Grok authentication,
hay chọn model Grok; cài Kit dùng signed remote registry theo mặc định trừ khi
...
| --- | --- | --- |
| `--adapter <ids>` | Runtime được phát hiện | Cấu hình tập adapter đã đăng ký, phân tách bằng dấu phẩy: `claude-code`, `codex`, `cursor`, `grok` hoặc `omp`. Projection Kit Pi được cài bằng `ak kit init --target pi`; nó không phải setup adapter ở đây. |
...
phân loại trạng thái đăng nhập native. Với `grok`, lệnh resolve
`AGENTKIT_GROK_BIN` hoặc `grok` nhưng để trạng thái auth ở mức unknown và do
runtime quản lý. Lệnh không liên hệ AgentKit registry, không xác minh entitlement
Kit trả phí, không ghi cache Kit và không cài Kit.
