...
được hay không. Setup không cài package Kit, probe hoặc ghi xác thực DeepSeek Harness
...
| --- | --- | --- |
| `--adapter <ids>` | Runtime được phát hiện | Cấu hình tập adapter đã đăng ký, phân tách bằng dấu phẩy: `claude-code`, `codex`, `cursor`, `dsh`, `grok`, `omp` hoặc `pi`. `pi` xuất hiện vì nó là emitter projection Kit (`ak kit init --target pi`); liệt kê ở đây không cung cấp `ak run --target pi`. |
