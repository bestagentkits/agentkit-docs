...
redactors before export or submission. They also pass through a strike pass for
values the secrets vault on this machine released, which leaves
`[redacted:secret:<NAME>]` markers. If some or all release manifests cannot be
read, that pass is incomplete: the result sets `dynamic_redaction_note`, human
output prints it as a warning, and the rendered Markdown carries it as a note. Limits are applied at 160 runes for the
...
diagnostics_attached, rendered_body, dynamic_redaction_note
```

`dynamic_redaction_note` appears only when released values were not fully checked.
