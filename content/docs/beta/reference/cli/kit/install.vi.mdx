...
Trên POSIX, Codex home do wrapper quản lý chỉ được alias leaf `hooks` cuối cùng
tới `~/.codex/hooks` canonical; command Hook, thao tác ghi, rollback và
ownership dùng identity canonical đó. Parent có link, target không canonical và
mọi link hay reparse point trên Windows vẫn không được hỗ trợ.
