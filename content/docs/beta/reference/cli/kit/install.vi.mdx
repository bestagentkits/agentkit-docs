...
<Callout type="warning">
  ...
  dùng cùng `--local --kits-dir <dir>`. Native installation đã được triển khai,
  nhưng chưa phát hành package Grok registry đã ký hoặc provider canary có xác
  thực; install Grok từ remote sẽ fail closed.
</Callout>
...
| `--target <ids>` | `claude-code` | Chọn một hoặc nhiều emitter target phân tách bằng dấu phẩy; `grok` được đăng ký cho nguồn local-development rõ ràng. |
...
| Grok Build trong project hoặc user (`spike`) | `--local --kits-dir <dir> --target grok` cùng `--global` nếu cần | `.grok` trong project hoặc `$GROK_HOME` (mặc định `~/.grok`) cùng lifecycle metadata. |
