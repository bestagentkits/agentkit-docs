...
Informational emit notices, including recorded paths left untouched because
they belong to a runtime home this environment does not manage, print as
`[i]` lines. They are never counted as attention items. A healthy install or
refresh that only has notices reports zero attention items.
