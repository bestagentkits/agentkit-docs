...
| Pi native spike | Runtime files under `.pi/skills`, `.pi/agents`, `.pi/rules`, and `.pi/extensions` | Active Pi coding-agent profile (`PI_CODING_AGENT_DIR` or `~/.pi/agent`) with no extra `.pi/` segment | Use `--target pi`; add `--global` only for an empty or AgentKit-owned active profile. |
