...
<Callout type="warning">
  ...
  combine `--local --kits-dir <dir>`. Native projection is implemented, but no
  signed Grok registry package or authenticated provider canary has shipped;
  remote Grok init fails closed.
</Callout>
...
| `--target <ids>` | `claude-code` | Select one or more comma-separated emitter target IDs; `grok` is registered for explicit local-development sources. |
...
| Grok Build native (`spike`) | Runtime files under `.grok` plus lifecycle metadata under `.agentkit/adapters/grok/<kit>/` | Runtime files under `$GROK_HOME` (default `~/.grok`) plus lifecycle metadata under `${AGENTKIT_HOME:-~/.agentkit}/adapters/grok/<kit>/` | Use `--local --kits-dir <dir> --target grok`; add `--global` for user scope. |
