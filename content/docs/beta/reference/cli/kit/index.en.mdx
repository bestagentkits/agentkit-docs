...
1. **Source:** authenticated remote release by default for published targets, or
   explicit `--local --kits-dir <dir>` for development and CI.
2. **Target:** `claude-code`, `codex`, `cursor`, or `grok` is a native signed registry target for Kit installation and refresh:
