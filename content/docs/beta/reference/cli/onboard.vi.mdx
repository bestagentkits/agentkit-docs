...
5. Resolve Kit và target, xác nhận chúng trên TTY trừ khi `--kit` hoặc `--target` đã khoá trục đó, rồi chỉ cài khi route đã chọn chưa có install receipt.
...
tác, onboard có thể đăng nhập và thử cài lại một lần. Nếu runtime đã có tệp
collision của AgentKit, TTY có thể xác nhận một lần retry ghi đè với `--force`;
tệp tuỳ chỉnh không thuộc AgentKit được giữ.

<Callout type="warning">
  Lần cài đầu bắt đầu ở chế độ preserve. `--yes` và chế độ không tương tác không
  bao giờ bật force. Chỉ xác nhận ghi đè tường minh trên TTY mới retry các đường
  dẫn collision với `--force`.
</Callout>
...
| Home của user trên TTY | Cảnh báo home không phải project, in bước tiếp theo `cd` / `--project-dir` / `--global`, rồi thoát `2`. |
...
giá trị trước khi cài trừ khi cờ tương ứng đã khoá trục đó. Khi không có TTY,
thiếu target sẽ thoát `2`.
