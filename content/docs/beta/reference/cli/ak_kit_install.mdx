...
Examples:
  ak kit init core --yes --no-interactive # remote is the default; offline/no-login falls back to embedded/local
  ak kit init core --local --yes          # dev/CI: force the embedded/local source, no network
...
```
      ...
      --channel string          Remote registry channel (default "beta")
  ...
  -h, --help                    help for install
      ...
      --local                   Install from the embedded/local kit source instead of the remote registry (dev/CI). Paid kits still require remote access (the default) or an explicit --kits-dir.
  ...
  -q, --quiet                   Suppress non-error output on stderr
      ...
      --remote                  Install from the authenticated remote registry (default; use --local for the embedded/dev source) (default true)
