...
`--api-key` and `--license-key` authenticate different account states, not
interchangeable credentials:

- **API key** (`ak_live_...`, from your web profile) — for kit owners: a
  direct purchase, a team seat, or a paid waitlist grant. It authenticates the
  CLI slot and is what `ak licenses` queries against.
- **License key** (`ak_license_...`, from `/account/licenses`) — for the
  AgentKit App. It activates an App device session and does not by itself
  grant a kit entitlement.

An account with only an AgentKit App license has no kit to authenticate the
CLI with `--api-key`; use `--license-key` for that account instead. See
[API-key login and creation errors](../../troubleshooting/authentication#api-key-login-and-creation-errors)
for the exact error codes and fixes.
