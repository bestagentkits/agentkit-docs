...
local binary resolves. Setup does not install Kit packages, probe or write Grok
authentication, or choose a Grok model; Kit installation uses the signed remote
...
| --- | --- | --- |
| `--adapter <ids>` | Detected runtimes | Configure a comma-separated set of registered adapters: `claude-code`, `codex`, `cursor`, `grok`, or `omp`. Pi Kit projection is installed with `ak kit init --target pi`; it is not a setup adapter here. |
