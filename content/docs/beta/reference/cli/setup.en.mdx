...
| --- | --- | --- |
| `--adapter <ids>` | Detected runtimes | Configure a comma-separated set of registered adapters: `claude-code`, `codex`, `cursor`, `dsh`, `grok`, `omp`, or `pi`. `pi` appears because it is a Kit projection emitter (`ak kit init --target pi`); listing it here does not provide `ak run --target pi`. |
