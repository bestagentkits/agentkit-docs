...
description: Chọn thao tác tra cứu, validate hoặc thay đổi Hook toggle, journal.auto, worktree.root, hay agent model có giới hạn, đồng thời giữ đúng scope, default, secret và ranh giới backup.
---
...
| Ghim một Hook toggle, bước journal tự động, worktree root mặc định, hoặc một agent model theo capability tier của runtime | `ak config prefs set <key> <value>` | Backup config đã chọn nếu tồn tại, rồi ghi atomically một boolean `hooks.<name>`, đúng leaf `journal.auto`, đúng path `worktree.root`, hoặc một model ID `agent_models.<runtime>.<tier>`. |
| Ngừng ghim một Hook toggle, `journal.auto`, `worktree.root`, hoặc agent model | `ak config prefs unset <key>` | Xoá một leaf `hooks.<name>`, `journal.auto`, `worktree.root`, hoặc `agent_models.<runtime>.<tier>` trong scope đã chọn khi tồn tại, có backup trước khi ghi lại. |
