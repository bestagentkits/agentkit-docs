...
description: Chọn thao tác tra cứu, validate hoặc thay đổi Hook toggle, journal.auto, worktree.root, agent model hay semantic_decision có giới hạn, đồng thời giữ đúng scope, default, secret và ranh giới backup.
---
...
| Ghim một Hook toggle, bước journal tự động, worktree root mặc định, một agent model theo capability tier của runtime, hoặc một thiết lập semantic decision | `ak config prefs set <key> <value>` | Backup config đã chọn nếu tồn tại, rồi ghi atomically một boolean `hooks.<name>`, đúng leaf `journal.auto`, đúng path `worktree.root`, một model ID `agent_models.<runtime>.<tier>`, hoặc một leaf `semantic_decision.*`. |
| Ngừng ghim một Hook toggle, `journal.auto`, `worktree.root`, agent model, hoặc thiết lập semantic decision | `ak config prefs unset <key>` | Xoá một leaf `hooks.<name>`, `journal.auto`, `worktree.root`, `agent_models.<runtime>.<tier>`, hoặc `semantic_decision.*` trong scope đã chọn khi tồn tại, có backup trước khi ghi lại. |

Các lệnh này không chỉnh provider key, API token, update setting hay section
khác do setup quản lý. Credential của semantic decision nằm trong biến môi
trường `TYPESAFE_API_KEY`, không bao giờ nằm trong config.
...
`semantic_decision` là ngoại lệ của thứ tự ưu tiên project trên user. Chỉ user
scope mới cấp được sự đồng ý dùng provider hoặc chọn thiết lập provider;
project scope chỉ có thể đặt `semantic_decision.enabled` hoặc một mục
`semantic_decision.consumers.<consumer>` thành `false`. Xem
[ak eval decision](../cli/ak_eval_decision) để biết cách quyền được resolve và
