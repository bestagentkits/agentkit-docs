...
description: Remove one scoped hook, journal.auto, worktree.root, agent model, or semantic_decision preference, understand fallback behavior, and recover from the reported config backup.
---

import { Callout } from 'fumadocs-ui/components/callout';

Use `ak config prefs unset` to remove an explicit `hooks.<name>` toggle, the
`journal.auto` leaf, the `worktree.root` leaf, one
`agent_models.<runtime>.<tier>` override, or one `semantic_decision` leaf from
one config scope. The
...
built-in location chain; an agent model override falls back to that adapter's
built-in catalog default).

`semantic_decision` does not follow that precedence. Removing the user
`semantic_decision.enabled` returns it to the default `false`, which disables the
provider. Removing a project `semantic_decision` restriction restores the user
policy for that project; it never grants consent.
...
The command requires exactly one positional argument. The key must be one of
`hooks.<name>` (where `<name>` is a lowercase kebab-case hook script name),
the exact literal `journal.auto`, the exact literal `worktree.root`, or
`agent_models.<runtime>.<tier>` where `<runtime>` is the closed set `cursor`
and `<tier>` is one of `inherit`, `sonnet`, `opus`, `fable`, or `haiku`, or
`semantic_decision.<leaf>` where `<leaf>` is `enabled`, `provider`, `model`,
`timeout_ms`, or `consumers.<consumer>` for `domain-routing`,
`fix-complexity`, or `orchestration-risk`. It cannot remove credentials or
values from other config sections.
...
AgentKit home, removes only that leaf, prunes a parent section such as `hooks`, `journal`,
`worktree`, or `semantic_decision` that the removal leaves empty, and
atomically rewrites the config. The success output
