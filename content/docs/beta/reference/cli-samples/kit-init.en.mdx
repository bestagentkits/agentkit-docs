...
snapshot fields. When a target cannot project one or more Hook groups, `data`
can also include the optional `hooksDropped` count and
`droppedHookSummaries` array.
...
The success summary reports installed Hook events and scripts when present. If
a target cannot project a Hook group, human output adds:

```text
Hooks dropped (unsupported on this target): <count> groups - see warnings
```

It then prints an actionable summary for each dropped group. This disclosure is
independent of the installed-Hook line, so it remains visible even when every
Hook group is dropped.

For Codex, the Engineer task-lifecycle `PostToolUse` matcher group can be
reported as not projected because Codex has no equivalent matcher. Compatible
Hook groups remain installed, and task-state refresh still runs at supported
`Stop` and `SubagentStop` boundaries. The install still succeeds: a disclosed
drop is a target limitation, not a repaired Hook or a fatal installation.
