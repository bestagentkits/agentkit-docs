...
description: Choose a preference lookup, validation, or bounded hook-toggle mutation while preserving scope, defaults, secrets, and backup boundaries.
---
...
| Pin one Hook toggle | `ak config prefs set <key> <true\|false>` | Backs up an existing selected config, then atomically writes one `hooks.<name>` boolean. |
| Stop pinning one Hook toggle | `ak config prefs unset <key>` | Removes one selected-scope leaf when present, with a backup before rewrite. |
...
Kit Hook applies its own built-in default when a value is absent.
...
- [Complete `ak config prefs` command facts](../cli/ak_config_prefs)
- [Resolve authored preferences](../cli/ak_config_prefs_resolve)
- [Set a Hook preference](../cli/ak_config_prefs_set)
- [Validate config files](../cli/ak_config_prefs_validate)
- [Remove a Hook preference](../cli/ak_config_prefs_unset)
