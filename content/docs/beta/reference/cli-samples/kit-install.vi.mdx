...
`hooksDropped` và mảng `droppedHookSummaries`. Đầu ra dành cho người dùng in
`Hooks dropped (unsupported on this target): <count> groups - see warnings`
cùng phần tổng hợp nêu hành động cho từng nhóm. Một nhóm bị loại đã được thông
báo không làm quá trình cài thất bại.
...
Hãy xem cảnh báo sau một lần cài thành công. Ví dụ, matcher `PostToolUse` cho
vòng đời task của Claude có thể được báo là không được chiếu, trong khi các nhóm
Hook tương thích cùng ranh giới refresh `Stop` và `SubagentStop` được hỗ trợ vẫn
hoạt động. Trên Windows, Hook Codex `.cjs` đủ điều kiện nằm trong Codex home có
