...
description: Chọn thao tác tra cứu, validate hoặc thay đổi Hook toggle có giới hạn, đồng thời giữ đúng scope, default, secret và ranh giới backup.
---
...
| Ghim một Hook toggle | `ak config prefs set <key> <true\|false>` | Backup config đã chọn nếu tồn tại, rồi ghi atomically một boolean `hooks.<name>`. |
| Ngừng ghim một Hook toggle | `ak config prefs unset <key>` | Xoá một leaf trong scope đã chọn khi tồn tại, có backup trước khi ghi lại. |
...
dụng default tích hợp khi không có giá trị.
...
- [Thông tin đầy đủ của lệnh `ak config prefs`](../cli/ak_config_prefs)
...
- [Xoá Hook preference](../cli/ak_config_prefs_unset)
