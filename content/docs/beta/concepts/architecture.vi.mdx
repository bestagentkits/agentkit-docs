...
flowchart TB
  ...
  local["Nguồn Kit cục bộ rõ ràng<br/>(development hoặc CI)"] --> cli
  ...
  adapter --> grok["Bề mặt native của Grok Build"]
  adapter --> omp["Oh My Pi package staging<br/>và dispatch"]
```
...
- **Runtime adapter** chuyển các bề mặt được hỗ trợ của Kit thành tệp và đăng ký
  mà Claude Code, Codex, Cursor, Grok Build hoặc Oh My Pi hiểu được. Adapter cũng báo
...
Với Grok Build native, `--target grok` chọn package registry đã ký, projection
và ranh giới dispatch của AgentKit. AgentKit sở hữu path đã cài cùng process
launch; Grok và người dùng sở hữu model thực tế, xác thực, trust của project,
quyền, sandbox và `.grok/config.toml`. Cài Grok từ nguồn cục bộ vẫn khả dụng cho
development hoặc CI bằng source `--local --kits-dir` rõ ràng.
