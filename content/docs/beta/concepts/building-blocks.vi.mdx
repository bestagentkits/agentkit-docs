...
```mermaid
flowchart TB
  accTitle: Cấu trúc của một Kit dành cho người dùng
  accDescr: Một Kit dành cho người dùng kết hợp core dùng chung nội bộ với nội dung riêng của Kit. Gói sau khi phân giải chứa Skills, Agents, Hooks và các artifact hỗ trợ.
  kit["Kit dành cho người dùng"] --> core["Core dùng chung (nội bộ)"]
  kit --> specific["Nội dung riêng của Kit"]
  core --> resolved["Các thành phần đã phân giải"]
  specific --> resolved
  resolved --> skills["Skills"]
  resolved --> agents["Agents"]
  resolved --> hooks["Hooks"]
  resolved --> support["Artifact hỗ trợ"]
```
