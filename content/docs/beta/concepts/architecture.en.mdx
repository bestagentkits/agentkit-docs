...
flowchart TB
  ...
  accDescr: An authenticated registry supplies verified Kit releases for released targets, while an explicit local source supplies Kits for development-only targets. The AgentKit CLI projects each Kit through a runtime adapter.
  registry["Authenticated registry"] --> release["Verified Kit release"]
  release --> cli["AgentKit CLI"]
  local["Explicit local Kit source<br/>(development or CI)"] --> cli
  ...
  adapter --> grok["Grok Build native surfaces"]
  cli --> omp["Oh My Pi harness dispatch<br/>(ak run --target omp)"]
```
...
- **A runtime adapter** translates supported Kit surfaces into the files and
  registrations understood by Claude Code, Codex, Cursor, or Grok Build. It also
  reports content that the target cannot activate. `ak run` can also dispatch
  local Skill execution through Oh My Pi with `--target omp`.
...
For native Grok Build, `--target grok` selects AgentKit's signed registry
package, projection, and dispatch boundary. AgentKit owns its installed paths
and process launch; Grok and the user own the effective model, authentication,
project trust, permissions, sandbox, and `.grok/config.toml`. Local-source Grok
installs remain available for development or CI with an explicit `--local
--kits-dir` source.
