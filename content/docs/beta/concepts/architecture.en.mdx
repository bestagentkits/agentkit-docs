...
flowchart TB
  ...
  adapter --> omp["Oh My Pi package staging<br/>and dispatch"]
```
...
- **A runtime adapter** translates supported Kit surfaces into the files and
  registrations understood by Claude Code, Codex, Cursor, Grok Build, or Oh My Pi.
  It also reports content that the target cannot activate. `ak run` can dispatch
  Skill execution through Oh My Pi with `--target omp`.
