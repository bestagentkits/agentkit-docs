...
<Callout type="info">
  These pages describe AgentKit Desktop **vAK_DESKTOP_VERSION**. Artifact filenames,
...
## Published vAK_DESKTOP_VERSION packages
...
No Desktop package was published for Linux ARM64 or Windows ARM64 in
vAK_DESKTOP_VERSION. The CLI remains available on both.
...
- List, verify, and preview supported MCP configuration changes before applying
  them;
- Store secrets in the vault, answer agent access requests, and review the
  secrets audit log (see [Secrets](./secrets));
- Ask questions about your local usage with your own Anthropic, OpenAI, or
  OpenRouter API key (see [Assistant](./assistant)); and
- Edit AgentKit configuration, inspect effective settings, and locate local
  AgentKit data stores.

Desktop vAK_DESKTOP_VERSION does not launch a Kit skill or agent run. Run the installed skill
