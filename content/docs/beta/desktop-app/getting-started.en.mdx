...
The scan also reports Node.js dependency readiness. If Node.js is missing,
Desktop shows an installation hint and an explicit Node.js LTS installation
action. Selecting that action grants consent to install through a detected
supported package manager; Desktop then resolves Node.js again and checks the
version against the `20.11` minimum. Without a supported manager, follow the
manual hint. Non-interactive mode does not grant consent; CLI or CI callers must
set `AK_AUTO_INSTALL_NODE=1` explicitly to authorize automatic installation.

The optional Kit step installs only the Kits you select, globally for the
detected coding agents, from the Kit catalog used by Desktop vAK_DESKTOP_VERSION. You can
...
using the new skills. Other Kit content can still be installed while Node.js is
missing, but Node-backed Hooks and status lines require it. After Node.js is
ready, rerun the affected Kit install or refresh action.

Desktop vAK_DESKTOP_VERSION does not start a Kit skill or agent run. Launch the installed
...
- App activation, entitlement checks, and Kit catalog actions contact AgentKit
  services;
- Update checks, changelog, and announcements contact the release service;
- Feedback is sent only when you submit it; and
- The **Your reports** list on Feedback reads each filed report's issue state
  from the public GitHub API, without authentication.
