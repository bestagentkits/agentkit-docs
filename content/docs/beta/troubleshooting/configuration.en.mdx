...
`set` also accepts the exact key `journal.auto` with a boolean. Setting it to
...
key, a non-boolean for `hooks.<name>` or `journal.auto` value, or a schema failure exits `1`.
