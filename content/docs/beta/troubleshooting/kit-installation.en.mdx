...
All three commands select released registry targets. The default scope is the current project. `--global` selects the
...
`claude-code`, `codex`, or `cursor`. If a Grok-specific Doctor filter is not
...
Run the adapter-specific doctor check again where one exists. For Grok, start a
