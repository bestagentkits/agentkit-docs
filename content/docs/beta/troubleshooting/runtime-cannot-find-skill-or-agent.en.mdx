...
For Oh My Pi, confirm both the Kit package staging route and the `ak run
<kit>/<skill> --target omp` dispatch path. Oh My Pi owns its configured backend
and credentials, so runtime discovery can differ from file presence.
...
| Oh My Pi | OMP package staging plus project lifecycle metadata | OMP package staging plus user-scope lifecycle metadata |
