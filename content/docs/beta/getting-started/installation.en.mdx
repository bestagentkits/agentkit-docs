...
run a kit Skill there. Released registry targets include Claude Code, Codex, Cursor, and Grok. Oh My
Pi is available as an `ak run --target omp` harness target for local Skill
dispatch. AgentKit account authentication is a separate step.
