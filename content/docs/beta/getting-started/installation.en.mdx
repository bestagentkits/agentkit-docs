...
Cursor, Grok, Pi (`pi`), and Oh My Pi (`omp`). Pi is an installable Kit
projection target; Oh My Pi can also be selected by `ak run --target omp` for
Skill dispatch. AgentKit account authentication is a separate step.
