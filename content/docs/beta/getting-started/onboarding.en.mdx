...
Claude Code, Codex, and Cursor project-native installs use the authenticated
remote registry and the `stable` kit channel by default. Grok Build uses the
explicit local-source spike route shown above. For Claude Code, plugin delivery
...
For Cursor and the Grok Build spike, verify the installed Skill through that
