...
không truyền `--worktree-root`. Ghi absolute path ở project scope trả exit `1` và không ghi file. Khi đọc project config đã tồn tại, resolver có thể cảnh báo và xét scope kế tiếp; config đã commit có thể đến từ clone không đáng tin. User scope chấp nhận absolute
