...
description: Choose a preference lookup, validation, or bounded Hook, journal.auto, worktree root, agent model, or semantic_decision mutation while preserving scope, defaults, secrets, and backup boundaries.
---
...
| Pin one Hook toggle, the automatic journal step, the default worktree root, one runtime capability-tier agent model, or one semantic decision setting | `ak config prefs set <key> <value>` | Backs up an existing selected config, then atomically writes one `hooks.<name>` boolean, the exact `journal.auto` leaf, the exact `worktree.root` path, one `agent_models.<runtime>.<tier>` model ID, or one `semantic_decision.*` leaf. |
| Stop pinning one Hook toggle, `journal.auto`, `worktree.root`, an agent model, or a semantic decision setting | `ak config prefs unset <key>` | Removes one selected-scope `hooks.<name>`, `journal.auto`, `worktree.root`, `agent_models.<runtime>.<tier>`, or `semantic_decision.*` leaf when present, with a backup before rewrite. |

These commands do not edit provider keys, API tokens, update settings, or other
setup-owned sections. The semantic decision credential stays in the
`TYPESAFE_API_KEY` environment variable, never in config.
...
`semantic_decision` is an exception to project-over-user precedence. Only user
scope can grant provider consent or choose provider settings; project scope can
only set `semantic_decision.enabled` or a
`semantic_decision.consumers.<consumer>` entry to `false`. See
[ak eval decision](../cli/ak_eval_decision) for how the grant is resolved and
what it sends.
...
- [Managing Hook preferences](../../../../guides/managing-hook-preferences)
