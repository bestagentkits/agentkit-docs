...
Released kits use the authenticated remote registry by default. When
`--channel` is omitted, `ak kit init` uses persisted `kit.channel` if that key
is set; otherwise it uses `stable`. Passing `--channel stable` or
`--channel beta` persists that value for later `ak kit init`, `ak kit refresh`,
and `ak update`. Paid kits require a valid CLI session and entitlement before
preview or writes.
...
the docs header. Omitting `--channel` uses persisted `kit.channel` when set,
otherwise `stable`.
...
| `--channel <channel>` | `stable`, or persisted `kit.channel` | Select the remote registry channel; an explicit value is persisted. |
...
| `--target <ids>` | `claude-code` | Select one or more comma-separated emitter target IDs, including `dsh`, `grok`, and `pi`; generated command facts may lag newly registered Kit projection targets. |
...
| DeepSeek Harness native | Skills under `.dsh/skills/` plus lifecycle metadata under `.agentkit/adapters/dsh/<kit>/` | Skills under `${DSH_HOME:-~/.dsh}/skills/` plus lifecycle metadata under `${AGENTKIT_HOME:-~/.agentkit}/adapters/dsh/<kit>/` | Use `--target dsh`; add `--global` for user scope. |
...
| Oh My Pi native | Portable `ak-<slug>` Skills under `.omp/skills`, plus project Agents, rules, and `AGENTS.md` | The corresponding surfaces below the resolved OMP profile root | Use `--target omp`; add `--global` for user scope. Project OMP also discovers project `.agents/skills`. |
...
For global Codex Hooks, an established wrapper may alias only the final `hooks`
leaf to the resolved user's canonical `~/.codex/hooks`, using a POSIX symlink or
a Windows directory junction. AgentKit uses that canonical path for commands,
writes, rollback, and ownership. Linked parents and leaves that resolve to any
other target are rejected.
...
OMP projection uses portable `ak-<slug>` directories and top-level
`name: ak-<slug>` in projected `SKILL.md` files. It does not rewrite body aliases
such as `/ak:cook` or `$ak:cook`, and `ak run <kit>/<skill> --target omp` keeps
its dispatch selector. In a trusted project, project `.omp/skills` shadows a
colliding project `.agents/skills` copy for project-scope OMP installs; project
`.agents/skills` shadows the OMP profile copy for global installs used from that
project. `ak kit init --target omp` warns and continues without skipping OMP
emission, deleting or rewriting either Skill, altering `.agents` or OMP settings,
requiring `--force`, or returning exit `6`.
