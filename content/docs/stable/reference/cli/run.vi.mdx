---
title: ak run
description: Resolve local Kit skill và stream qua Claude Code, Codex, Cursor, DeepSeek Harness, Grok Build hoặc Oh My Pi với hành vi stdin, timeout, output và dependency rõ ràng.
---
...
| Cờ | Mặc định | Mô tả |
| --- | --- | --- |
| `--target <adapter>` | `claude-code` | Chọn `claude-code`, `codex`, `cursor`, `dsh`, `grok` hoặc `omp`. |
...
Route một invocation qua adapter DeepSeek Harness hoặc Grok đã đăng ký, hoặc qua
harness dispatch target Oh My Pi. Projection Kit Pi không phải adapter `ak run`
trong bản phát hành này; dùng `ak kit init ... --target pi` cho Skill cài vào Pi
và dùng discovery riêng của Pi để gọi chúng.
...
| `dsh` | `AGENTKIT_DSH_BIN` hoặc `dsh` trên `PATH`. | DeepSeek Harness quản lý model, credential và quyền thực thi. |
...
| `omp` | `AGENTKIT_OMP_BIN` hoặc `omp` trên `PATH`; chạy `omp -p --mode json --no-session --cwd <cwd> @<prompt-file>`. | Bỏ provider gate; Oh My Pi sở hữu backend và credential đã cấu hình. |
...
<Callout type="warning">
  ...
  là harness dispatch cục bộ qua Oh My Pi. Tuyến này tách biệt với các
  projection Kit có thể cài, gồm `ak kit init <kit> --target pi` và
  `ak kit init <kit> --target omp`, và không thay thế bản cài Kit ở project hoặc
  global.
</Callout>

<Callout type="warning">
  DeepSeek Harness chạy dispatch headless one-shot. AgentKit truyền prompt đã tạo
  qua process argument, vì vậy không đưa secret vào yêu cầu Skill hoặc argument.
</Callout>
...
Với OMP, AgentKit ghi prompt đã tạo vào file tạm mode `0600`, chỉ truyền tham
chiếu `@<path>` của file cho process rồi xóa file khi process kết thúc. Cơ chế
này giữ nội dung prompt khỏi danh sách argument của process; nó không thay đổi
ranh giới truyền tới provider hoặc redaction ở trên.
