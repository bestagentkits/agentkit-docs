...
Who it's for:
  Power users and maintainers who want to inspect how skills connect across kits. The dashboard Skills page exposes the same graph visually.

When to use it:
  Use before editing skill workflow metadata, while auditing skill discovery, or when checking why one skill should typically precede or follow another.
...
What changes on disk:
  Read-only. Loads local kit metadata and skill markdown through KitSource; writes nothing.
