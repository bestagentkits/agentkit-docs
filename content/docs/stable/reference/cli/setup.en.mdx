...
| --- | --- | --- |
| `--adapter <ids>` | Detected runtimes | Configure a comma-separated set of registered adapters. |
| `--no-adapter <ids>` | Empty | Exclude a comma-separated set of registered adapters. |
...
timeouts to classify native login state. It does not contact the AgentKit
registry, validate paid-kit entitlement, populate the kit cache, or install a
kit.
