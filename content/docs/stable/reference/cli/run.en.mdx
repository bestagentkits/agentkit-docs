...
Route one invocation through the registered Grok adapter, or through the Oh My Pi local harness dispatch target. Pi Kit projection is not an `ak run` adapter in this release; use `ak kit init ... --target pi` for Pi-installed Skills and Pi's own discovery to invoke them.
