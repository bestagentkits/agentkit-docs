---
title: ak run
description: Resolve a local Kit skill and stream it through Claude Code, Codex, Cursor, DeepSeek Harness, Grok Build, or Oh My Pi with explicit stdin, timeout, output, and dependency behavior.
---
...
| --- | --- | --- |
| `--target <adapter>` | `claude-code` | Select `claude-code`, `codex`, `cursor`, `dsh`, `grok`, or `omp`. |
...
Route one invocation through the registered DeepSeek Harness or Grok adapter, or
through the Oh My Pi local harness dispatch target. Pi Kit projection is not an
`ak run` adapter in this release; use `ak kit init ... --target pi` for
Pi-installed Skills and Pi's own discovery to invoke them.

```bash
ak run engineer/scout --target grok
ak run engineer/scout --target dsh
...
| `dsh` | `AGENTKIT_DSH_BIN` or `dsh` on `PATH`. | DeepSeek Harness owns model, credentials and permissions. |
...
<Callout type="warning">
  ...
  separate from installable Kit projections, including `ak kit init <kit>
  --target pi` and `ak kit init <kit> --target omp`, and does not replace a
  project or global Kit install.
</Callout>

<Callout type="warning">
  DeepSeek Harness runs one-shot headless dispatch. AgentKit passes the generated
  prompt as a positional process argument, so do not place secrets in the Skill
  request or its arguments.
</Callout>
