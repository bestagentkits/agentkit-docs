...
5. Resolve the Kit and target, confirm them on a TTY unless `--kit` or `--target` locked that axis, then install only when the selected route has no install receipt.
...
once. If the runtime already has AgentKit collision files, a TTY can confirm one
overwrite retry with `--force`; unrelated custom files stay.

<Callout type="warning">
  First installation starts preserve-only. `--yes` and non-interactive mode never
  enable force. Only an explicit TTY overwrite confirmation retries occupied
  collision paths with `--force`.
</Callout>
...
| User home on a TTY | Warns that home is not a project, prints `cd` / `--project-dir` / `--global` next steps, and exits `2`. |
...
then the first enabled adapter. On a TTY, onboard confirms both values before
install unless the matching flag already locked that axis. Without a TTY, a
