...
| --- | --- | --- |
| `--docs-dir <path>` | Unset | Persist the documentation directory as `paths.docs`. |
| `--plans-dir <path>` | Unset | Persist `paths.plans`; use a relative path for compatibility with plan readers. |
...
A relative `--plans-dir` is honored by Skill, `ak plan`, journals and the dashboard. An absolute value is honored only by config and Skill.
