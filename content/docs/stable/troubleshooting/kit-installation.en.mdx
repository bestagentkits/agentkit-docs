...
For global Codex Hooks, do not treat every symlink as interchangeable. On POSIX,
only the final `hooks` leaf may point to the resolved user's canonical
`~/.codex/hooks`; linked parents and other targets are refused. Windows
symlinks and reparse points remain unsupported. Resolve the path mismatch
