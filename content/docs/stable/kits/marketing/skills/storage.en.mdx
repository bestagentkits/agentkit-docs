...
`ak:ckm-storage` is retired and no longer packaged as a separate router;
continue with `ak:storage` for this client integration.

<Callout type="info" title="Invoke the integration by its canonical name">
  Use `/ak:storage` in Claude Code or Cursor and `$ak:storage` in Codex.
</Callout>
...
These are integration client functions, not router-style user-facing
subcommands. Upload and sync infer common content types; unknown extensions
...
| --- | --- |
| `/ak:storage` or `$ak:storage` is not discovered | Confirm the installed `ak-storage` export declares `name: ak:storage`. |
...
- `ak:storage` identifies the integration; the retired `ak:ckm-storage` router
  no longer exists as a separate identity.
