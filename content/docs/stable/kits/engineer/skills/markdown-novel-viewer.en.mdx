...
6. **The browser loads enhancements.** Local reader JavaScript and shipped
   `assets/styles/` theme modules add theme, font, sidebar, keyboard, mobile,
   and progress behavior. The entry stylesheet must ship with every module it
   imports. External CDN resources still add fonts, Highlight.js theme CSS, and
   Mermaid.
...
| Fonts or syntax colors are missing offline | Continue with the local reader layout and bundled theme modules, or approve the documented Google Fonts and cdnjs requests; Highlight.js themes and Mermaid are not bundled. |
