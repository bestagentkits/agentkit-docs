...
Mô tả topology và vai trò ngữ nghĩa; compiler bố trí theo lớp và định tuyến cạnh vuông góc. Đọc legend để hiểu vai trò node. Entrance mặc định hữu hạn, kết thúc dưới tám giây; trace tùy chọn cũng hữu hạn. Dùng Replay hoặc `M` để phát lại; reduced-motion hiển thị thẳng frame cuối. `--step` là CSS custom property nội bộ, không phải flag của CLI `ak`.
