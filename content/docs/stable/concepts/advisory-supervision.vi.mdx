...
Topology tối thiểu nên ghi rõ policy dành cho subagent:
...
Mặc định, OMP không chạy native advisor bên trong subagent. `task.agentAdvisor`
điều khiển nested advisor theo từng Agent; setting này không chọn primary model
của Agent. Vì vậy `kongming: "off"` là dư thừa nếu không có Agent frontmatter
hoặc lớp cấu hình khác bật nó, nhưng giúp topology dự kiến trở nên rõ ràng. Không
dùng setting legacy `advisor.subagents` cho mục đích này: giá trị `true` chỉ
migrate bundled Agent `task` sang `task.agentAdvisor: { task: "on" }`, không
bật mọi Agent và không bật `kongming`.

Trên OMP, AgentKit map tier `fable` của `kongming` thành
`model: "@advisor"`. Nếu không override, native default advisor và `kongming`
đều resolve qua `modelRoles.advisor`. Chúng vẫn tạo request và context riêng,
nhưng không tạo model diversity.

Để dùng primary model riêng cho `kongming`, override Agent đã được project:
...
`task.agentModelOverrides.kongming` có precedence cao hơn
`model: "@advisor"` trong Agent đã được project. Giữ
`task.agentAdvisor.kongming` ở `off`, trừ khi bạn chủ động muốn có thêm một
native OMP advisor bên trong mỗi spawned `kongming` session. Nếu
`modelRoles.advisor` bị thiếu hoặc không khả dụng, native default advisor và
`kongming` chưa được override sẽ không resolve được model; explicit model trong
roster và explicit override cho `kongming` không phụ thuộc role này.
...
| Codex | AgentKit map `kongming` sang frontier model override của Codex với reasoning effort theo Kit. Frontmatter `fable` dùng chung vẫn portable, nhưng model ID riêng của Codex chỉ do adapter Codex emit. `/ak:advise --agent` không đồng nghĩa có relay path của Claude Code; dùng inline `ak:advise` trừ khi runtime đã cài báo hỗ trợ. |
...
và reasoning effort theo Kit, trong khi vẫn giữ source chung `model: fable` portable.
