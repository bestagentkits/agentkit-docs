...
A minimal separation of roles looks like this:
...
Keep `advisor.subagents: false` when first combining the two mechanisms. Turning
it on also enables the native advisor inside eligible spawned subagent sessions,
which can make the background reviewer supervise `kongming` itself. That extra
layer is usually redundant and adds cost and latency.

If both mechanisms resolve through the same advisor role, they still make
separate requests with separate contexts, but they do not provide model
diversity. If the advisor role is missing or unavailable, treat the native
advisor and any affected `kongming` checkpoint as not completed; do not report
that two independent reviews occurred.
