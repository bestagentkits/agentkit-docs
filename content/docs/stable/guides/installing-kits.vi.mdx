...
## Đọc cảnh báo Hook của Codex

Một lần cài Codex thành công có thể cảnh báo rằng một nhóm Hook không được hỗ
trợ đã bị bỏ qua. Codex không biểu diễn được matcher trạng thái task
`Agent|Task|TodoWrite|TodoRead` trong nhóm `PostToolUse` của Engineer Kit, nên
nhóm đó không được phát sang Codex. Các nhóm Hook tương thích vẫn được cài, còn
việc làm mới trạng thái task vẫn chạy ở các ranh giới `Stop` và `SubagentStop`
được hỗ trợ.

Cảnh báo này mô tả giới hạn capability của runtime, không phải lỗi cài đặt; cài
