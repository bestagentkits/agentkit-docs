...
## Read Codex Hook warnings

A successful Codex install can warn that an unsupported Hook group was omitted.
Codex cannot represent the Engineer Kit's `PostToolUse` task-state matcher
...
This warning describes a runtime capability limit, not an install failure;
reinstalling with `--force` does not add the unsupported matcher.
