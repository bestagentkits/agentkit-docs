...
Installs to `~/.local/bin` on the **stable** channel by default. The installer
does not modify your shell profile — add `~/.local/bin` to `PATH` if `ak`
...
itself. Always pass `--channel stable` on the stable docs path — an empty
