...
Examples:
  ...
  ak setup --adapter claude-code,codex,cursor,grok --no-interactive   # scripted adapter selection
...
```
      --adapter string      Comma-separated list of adapters to configure. Registered choices: claude-code, codex, cursor, grok. Defaults to locally resolved adapters (PATH or runtime override).
