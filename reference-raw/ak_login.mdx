...
What changes on disk:
  Writes a CLI registry session for email/API-key login or an App device session for license-key activation to ~/.agentkit/auth/session.json. Raw credentials are not persisted.
