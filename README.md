...
[CLI reference ownership](docs/workflows/cli-reference-pipeline.md).

Desktop release values are resolved at build time from `channels.json` and
`release-evidence/desktop/<tag>.json`; see the [release runbook](docs/workflows/release-and-deploy.md).
...
3. **Stable promote:** first verify the target Stable release against its exact
   ...
   artifact matrices for the runtime cohort of each bound release, including every
