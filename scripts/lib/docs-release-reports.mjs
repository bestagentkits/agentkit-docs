...
export async function writeV0Reports({ ledger, impactMap, outputRoot, target, ownerPaths = [], ownerActions = [] }) {
  const outputDir = releaseOutputDir(outputRoot, target);
  await assertNoSymlinkPath(outputRoot, outputDir);
  const request = createApprovalRequest({ ledger, impactMap, target, ownerPaths, ownerActions });
  const files = new Map([
