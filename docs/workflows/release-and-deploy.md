...
1. **Human-owned CLI prose** under `content/docs/beta/reference/cli/**/*.mdx`
   ...
   request records them as `ownerDirectedPaths` and binds them into its ID and
   digest; the impact map remains blocked to preserve the distinction between
   source evidence and owner-directed scope. The file may name only existing,
   modify-only human-owned Beta prose or metadata paths.
